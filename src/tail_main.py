...
def run_tail_pick(config=None, save=True, show_detail=True, push=False, use_theme=True):
    """
    运行尾盘选股
    ...
    dynamic_components = {}
    if use_theme:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 计算板块热度...")
        try:
            crawler = EastmoneyThemeCrawler()
            engine = ThemeHeatEngine(data_source=crawler)
            hot_df, components_dict = engine.get_hot_themes_with_components(top_n=10)
            if not hot_df.empty:
                # 构建板块热度字典 (归一化为 0-100)
                for _, row in hot_df.iterrows():
                    theme_heat_dict[row['板块名称']] = min(100, row['热度评分'])
                hot_themes_list = hot_df['板块名称'].head(5).tolist()
                dynamic_components = components_dict
                print(f"  热门板块 TOP5: {', '.join(hot_themes_list)}")
        except Exception as e:
            print(f"  板块热度计算失败：{e}，使用默认热门板块")

    if all_data is not None and not all_data.empty:
        picker.set_fund_flow_data(fund_flow)
        # 设置板块热度数据
        if use_theme and theme_heat_dict:
            picker.set_theme_heat(theme_heat_dict)
            # 动态更新热门板块列表
            picker.HOT_THEMES = hot_themes_list
            # 设置动态成分股
            picker.set_dynamic_components(dynamic_components)
        # 执行选股
    ...
    if save:
        ...
        # 显示推送格式（显示题材）
        print("\n" + exporter.export_for_push(result, top_n=10, show_themes=use_theme))
...
def quick_pick():
    """快速选股（一键运行）"""
    return run_tail_pick(save=True, show_detail=True, push=False, use_theme=True)
